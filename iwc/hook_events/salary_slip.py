...
class OverrideSalarySlip(SalarySlip):
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			...
			for data in timesheets:
				self.append("timesheets", {"time_sheet": data.name, "working_hours": data.total_hours, "regular_hours": data.regular_hours, "ot_hours": data.ot_hours})
	...
	def pull_sal_struct(self):
		...
		if self.salary_slip_based_on_timesheet:
			...
			self.regular_hours = sum([d.regular_hours or 0.0 for d in self.timesheets]) or 0.0
			self.ot_hours = sum([d.ot_hours or 0.0 for d in self.timesheets]) or 0.0
		...
		if self.timesheets:
			...
			for timesheet in self.timesheets:
				if timesheet.working_hours:
					self.regular_hours += timesheet.regular_hours
					self.ot_hours += timesheet.ot_hours
...
def set_missing_values(time_sheet, target):
	...
	target.regular_hours = doc.regular_hours
	target.ot_hours = doc.ot_hours
	target.append("timesheets", {"time_sheet": doc.name, "working_hours": doc.total_hours, "ot_hours": doc.ot_hours, "regular_hours": doc.regular_hours})
